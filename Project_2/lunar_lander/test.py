...
def get_checkpoint_path(opt, checkpoint_dir):
    ...
    if opt.checkpoint is not None:
        ...
        if os.path.isfile(os.path.join(checkpoint_dir, 'final.pth')):
...
def main(opt):
    ...
    if opt.gif:
        ...
        junk_files = [fname for fname in os.listdir(checkpoint_dir) if
                      fname.endswith('.mp4') or fname.endswith('.json')]
        for junk in junk_files:
            fname = os.path.join(checkpoint_dir, junk)
            os.remove(fname)


def fetch_agent_env_from_checkpoint(opt):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--checkpoint", type=str, help="Name of *.pth file")
