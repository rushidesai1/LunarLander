...
alphas = np.arange(0.0, 0.7, 0.01)
lambdas = [0, 0.3, 0.8, 1.0]

colors = ['salmon', 'mediumorchid', 'mediumslateblue', 'green']
...
rmse_file_name = "RMSE_alpha_lambda_error_seed_30.pkl"
if use_local:
    a_file = open(rmse_file_name, "rb")
    RMSE_vector = pickle.load(a_file)
else:
    # Generate a random walk
    generated_training_set = random_walks(7, 3, training_sequences, training_set_size, seed=30)

    # Setup initial RMSE vector
    # RMSE_vector = np.zeros(100 + 1)
    RMSE_vector = np.zeros((len(lambdas), len(alphas)))
    ...
    # Quoting sutton page 21:
    # "We presented the same data to the learning procedures again for several values of λ"
    # So we re-use same data-set generate above across lambdas.
    for ix, lambda_it in enumerate(lambdas):
        print('lambda = ' + str(lambda_it))
        for ii, alpha in enumerate(alphas):  # Iterate over each alpha value
            # for iteration in range(iterations):
            # while True:
            #     max_diff = float('-inf')

            # Quoting sutton page 21
            # "The Second Experiment concerns the question of learning rate when the training set is presented just
            # once rather than repeatedly until convergence."
            # The means we only need to present training examples once vs as we did in lambda_error.py
            for training_set in range(training_set_size):
                # Reset weights, now we update weight at end of every training_set.
                # Quoting Sutton page 22
                # "Fourth, so that there was no bias either toward rightside or leftside terminations all components
                # of the weight vector were initially set to 0.5"
                weights = 0.5 * np.ones(5)
                for training_seq in range(training_sequences):
                    # Update without waiting
                    # Quoting sutton on page 22
                    # "Second weight up dates were performed after each sequence rather than after each
                    # complete training set"
                    weights += learn_weight_updates(training_seqs=generated_training_set[training_set][training_seq],
                                                    lambda_val=lambda_it,
                                                    alpha=alpha, z_vals=(0, 1), w=weights)
                    # Add the new RMSE to the RMSE matrix
                    RMSE_vector[ix, ii] += np.sqrt(((np.round(weights, decimals=20) -
                                                     actual_values) ** 2).mean())

                    # single pass of training set completed, now update the weights
                    # weights += deltas
                    # max_diff = max(max_diff, abs(np.max(weights)))
    ...
    RMSE_vector /= training_set_size * training_sequences

    filehandler = open(rmse_file_name, 'wb')
    pickle.dump(RMSE_vector, filehandler)
    filehandler.close()

# Plot RMSE vs α for each λ
for ix, lambda_pl in enumerate(lambdas):
    plt.plot(alphas, RMSE_vector[ix, :], label='λ = ' + str(lambda_pl),
             color=colors[ix])
...
plt.ylabel('Error')
plt.legend(loc='best')
plt.title('Replication of Figure 4 in Sutton (1988)')
